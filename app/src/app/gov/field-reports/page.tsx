    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Reports filed by inspectors that aren&rsquo;t tied to an existing case assignment.
        </p>
      </div>
      <Card>
        <CardContent className="divide-y divide-border p-0">
          {inspections.length === 0 && <p className="px-4 py-6 text-sm text-muted-foreground">No standalone field reports yet.</p>}
              ...
              <div key={insp.id} className="flex items-start justify-between gap-4 px-4 py-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {inspector?.full_name ?? `user #${insp.inspector_user_id}`} &middot;{" "}
