  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">Public Complaints</h1>
          <p className="text-sm text-muted-foreground">Anonymously submitted concerns, awaiting triage.</p>
        </div>
      ...
      <Card>
        <CardContent className="divide-y divide-border p-0">
          {complaints.length === 0 && <p className="px-4 py-6 text-sm text-muted-foreground">No complaints match this filter.</p>}
          {complaints.map((c) => {
              ...
              <Link key={c.id} href={`/gov/complaints/${c.id}`} className="flex items-center justify-between gap-4 px-4 py-3 hover:bg-surface-inset">
                <div>
                  <div className="text-sm font-medium">
                    {c.category} &middot; {mine?.name ?? c.mine_id}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {c.id} &middot; {new Date(c.created_at).toLocaleDateString()}
                  </div>
                </div>
                <Badge className={STATUS_STYLE[c.status] ?? ""}>{c.status.replace(/_/g, " ")}</Badge>
              </Link>
            );
          })}
        </CardContent>
