...
export default async function MineDetailPage({ params }: { params: Promise<{ mineId: string }> }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="flex items-center justify-between py-3">
            <span className="text-sm">
              {t("This mine has an open case:")} <span className="font-mono">{compliance.case_id}</span>
            </span>
            <Link
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("Open findings ({count})", { count: openFindings.length })}</CardTitle>
        </CardHeader>
        <CardContent className="divide-y divide-border p-0">
          {openFindings.length === 0 && <p className="px-4 py-6 text-sm text-muted-foreground">{t("No open findings.")}</p>}
          {openFindings.map((f) => (
            <div key={f.id} className="flex items-start justify-between gap-4 px-4 py-3">
              <div>
                <div className="text-sm">{f.description}</div>
                <div className="mt-1 text-xs text-muted-foreground">
                  {formatRuleId(f.rule_id)} &middot; {t(DOMAIN_LABEL[f.domain] ?? f.domain)} &middot; {f.source_type}
                </div>
              </div>
      ...
      <Card>
        ...
        <CardContent className="divide-y divide-border p-0">
          {documents.length === 0 && <p className="px-4 py-6 text-sm text-muted-foreground">{t("No documents uploaded.")}</p>}
          {documents.map((d) => (
            <div key={d.id} className="flex items-center justify-between gap-4 px-4 py-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {d.extracted_permit_number ?? t("No permit number extracted")}
                  {d.extracted_expiry_date && ` · ${t("expires")} ${d.extracted_expiry_date}`}
                </div>
              </div>
      ...
      {satellite && (
        <Card>
          <CardHeader>
            <CardTitle>{t("Satellite comparison")}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="mb-3 text-sm">{satellite.description}</p>
            <div className="grid grid-cols-2 gap-3">
              <figure>
                <img src={satellite.before_image} alt={t("Before")} className="w-full rounded border border-border" />
                <figcaption className="mt-1 text-xs text-muted-foreground">{t("Before")} &mdash; {satellite.before_date}</figcaption>
              </figure>
              <figure>
                <img src={satellite.after_image} alt={t("After")} className="w-full rounded border border-border" />
                <figcaption className="mt-1 text-xs text-muted-foreground">{t("After")} &mdash; {satellite.after_date}</figcaption>
              </figure>
            </div>
            <p className="mt-3 text-xs text-muted-foreground">{t("NDVI vegetation loss:")} {satellite.ndvi_loss_pct}%</p>
          </CardContent>
        </Card>
      )}
    </div>
